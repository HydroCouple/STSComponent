...
HydraulicsBC::HydraulicsBC(Element *startElement, Element *endElement, int variableIndex, STSModel *model)
...
void HydraulicsBC::findAssociatedGeometries()
{
  ...
  if(m_variableIndex == 3)
  {
    for(size_t i = 0; i < m_profile.size(); i++)
    {
      m_profile[i]->xSectionArea.isBC = true;
    }
  }
}
      ...
      if(m_timeSeries->interpolate(dateTime, i, m_dataCursor, value))
      {
        switch (m_variableIndex)
        {
          ...
          case 2:
            ...
            break;
          case 3:
            m_profile[i]->xSectionArea.value = value;
            break;
        }
      }
    }
